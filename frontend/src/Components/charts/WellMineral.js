...
const WellMineral = (props) => {
  const chartRefs = useRef([]);
  const chartInstances = useRef([]);
  
  const elements = ["Na","Ca","Mg"];
  ...
  console.log("selected Water Sample",props.selectedSample);
  ...
  useEffect(() => {
    ...
    const createChart = (canvas, index) => {
      ...
      const quality = elements.map(element => {
        const value = samples[index]?.[element];
  ...
  return (
    <div className="w-full">
      <div className="p-4 bg-white border border-gray-200 rounded-lg shadow-sm">
          <h2 className="text-lg font-bold mb-4 text-center">Water Analysis</h2>
